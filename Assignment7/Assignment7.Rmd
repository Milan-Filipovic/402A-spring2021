...
A one-way anova to compare cytokine concentrations for different timepoints.  
```{r}
responseList <- names(isi_tble)[-c(1:6)]
modelList    <- lapply(responseList, function(resp) {
                           mF <- formula(paste(resp, " ~ visittype"))
                           aov(mF, data = isi_tble)
                })
for(i in c(1:23)){
...
A two-way anova will will compare cytokine concentrations by ISI status and different timepoints. 
```{r}
formula <- as.formula(paste0("cbind(", paste(names(isi_tble)[-c(1:6)], collapse = ","), ") ~ visittype"))

fit <- aov(formula, data=isi_tble)
summary(fit)
```

Two-way Interaction Plot. Here median values are plotted over the visit types. It should be notes that CYTO4, CYTO16, and CYTO17 were not included because significance was not found in the anova test. 
```{r}

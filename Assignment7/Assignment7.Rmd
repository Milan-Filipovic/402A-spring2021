...
To address what cytokines in the pre-transplant blood sample specifically correlate with immune cell infiltration after transplant and dead cells in the liver after transplant, a series of models were conducted using both inflammation score and necrosis score before reaching the best fitted model: ordinal logistic regression model, binary response model, and stepwise regression model.
...
```{r, message = F, echo=F}
options(warn = -1)  ##suppress warnings for this session 
...
### Histogram By Visit Type Interpretation 

Only CYTO4, CYTO16, and CYTO17 concentrations are not significantly different at the different visits by one way anova. In all other cytokines, an overall difference in mean concentration was detected. The overall mean at all timepoints is displayed with a horizontal line. 
...
main.table <- main.mod.17 %>% 
  tbl_regression(include = c("CYTO1", "CYTO16", "CYTO18", "CYTO22"), 
                 intercept = T, exponentiate = T) %>% 
  bold_p(t = 0.1)
...
# CYTO23 0.1
po.table <- po.mod.17 %>% 
  tbl_regression(include = c("CYTO3", "CYTO15", "CYTO16", "CYTO23"), 
                 intercept = T, exponentiate = T) %>% 
  bold_p(t = 0.1)
...
# CYTO14 0.1
lf.table <- lf.mod.17 %>% 
  tbl_regression(include = c("CYTO1", "CYTO2", "CYTO11", "CYTO13", "CYTO14"), 
                 intercept = T, exponentiate = T) %>% 
  bold_p(t = 0.1)
...
# CYTO22 0.05
w1.table <- w1.mod.17 %>% 
  tbl_regression(include = c("CYTO9", "CYTO13", "CYTO18", "CYTO22"), 
                 intercept = T, exponentiate = T) %>% 
  bold_p(t = 0.1)
...
changemain.table <- changemain.mod.17 %>% 
  tbl_regression(include = c("CYTO9.diffs"), 
                 intercept = T, exponentiate = T) %>% 
  bold_p(t = 0.1)
...
changemain2.table <- changemain2.mod.17 %>% 
  tbl_regression(include = c("CYTO2.diffs", "CYTO9.diffs"), 
                 intercept = T, exponentiate = T) %>% 
  bold_p(t = 0.1)
...
Correlation Plots
```{r corr plot, message = F, warning=F}
library(ggcorrplot)
...
For the logistic regression model containing main effects for all log-transformed cytokines (except CYTO17), we see significant coefficients for CYTO1, CYTO16, CYTO18, and CYTO22 at the $\alpha = 0.1$ level. We can interpret the coefficients as follows (using CYTO22 as example): for every 1 unit increase in CYTO22, the odds of being IRI+ increase by 1.51 times.

Logistic regression models for main effects were also stratified by visit type (PO, LF, and W1).
For the logistic regression model containing main effects for all log-transformed cytokines (except CYTO17) for only PO visits, we see significant coefficients for CYTO3, CYTO15, CYTO16, and CYTO23 at the $\alpha = 0.1$ level. We can interpret the coefficients as follows (using CYTO23 as example): for every 1 unit increase in CYTO23, the odds of being IRI+ increase by 2.15 times.

For the logistic regression model containing main effects for all log-transformed cytokines (except CYTO17) for only LF visits, we see significant coefficients for CYTO1, CYTO2, CYTO11, CYTO13, and CYTO14 at the $\alpha = 0.1$ level. We can interpret the coefficients as follows (using CYTO14 as example): for every 1 unit increase in CYTO14, the odds of being IRI+ increase by 0.29 times (decrease by 71%).

For the logistic regression model containing main effects for all log-transformed cytokines (except CYTO17) for only W1 visits, we see significant coefficients for CYTO9, CYTO13, CYTO18, and CYTO22 at the $\alpha = 0.1$ level. We can interpret the coefficients as follows (using CYTO22 as example): for every 1 unit increase in CYTO22, the odds of being IRI+ increase by 4.73 times.

#### Stepwise Main Effects + Interactions Logistic Regression Models

The significant predictors from the main effects model were then utilized to fit a logistic regression model including 2-way interactions for those predictors. Then, stepwise selection was conducted to find the logistic regression model with the lowest AIC while retaining meaningful predictors. 
For the stepwise selection of the logistic regression model containing main + interaction effects for all log-transformed cytokines (except CYTO17), we see significant coefficients for the intercept, CYTO16 and CYTO22 at the $\alpha = 0.1$ level (no significant interactions). We can interpret the coefficients as follows (using CYTO22 as example): for every 1 unit increase in CYTO22, the odds of being IRI+ increase by 1.24 times.

For the stepwise selection of the logistic regression model containing main + interaction effects for all log-transformed cytokines (except CYTO17) for only PO visits, we see significant coefficients for the intercept and the interaction CYTO3:CYTO15 at the $\alpha = 0.1$ level. Because there is a significant interaction, it may be possible that CYTO3 and CYTO15 vary together, although neither CYTO3 or CYTO15 are significant at the $\alpha = 0.1$ level.

For the stepwise selection of the logistic regression model containing main + interaction effects for all log-transformed cytokines (except CYTO17) for only LF visits, we see significant coefficients for the intercept, CYTO2 and CYTO11 at the $\alpha = 0.1$ level (no significant interactions). 

For the stepwise selection of the logistic regression model containing main + interaction effects for all log-transformed cytokines (except CYTO17) for only W1 visits, we see significant coefficients for the intercept, CYTO9, CYTO18, CYTO22, the interaction CYTO13:CYTO18, and the interaction CYTO13:CYTO22 at the $\alpha = 0.1$ level. Because there are significant interactions, it is possible that CYTO13, CYTO18, CYTO22 vary together at this time point.

#### Logistic Regression Using Log Differences in Consecutive Visits

For the logistic regression model containing main effects for change in log-transformed cytokines (except CYTO17) for PO to LF and LF to W1, we see significant coefficients for CYTO9 at the $\alpha = 0.1$ level. We can interpret the coefficients as follows: for every 1 unit increase in the change of CYTO9, the odds of being IRI+ increase by 0.65 times (decrease by 35%).

For the logistic regression model containing main effects for change in log-transformed cytokines (except CYTO17) from PO to LF, we see significant coefficients for the intercept, CYTO2, CYTO3, CYTO4, CYTO9, CYTO10, CYTO14, and CYTO19 at the $\alpha = 0.1$ level. We can interpret the coefficients as follows (using CYTO19 as example): for every 1 unit increase in the change of CYTO19, the odds of being IRI+ increase by 0.30 times (decrease by 70%).

For the logistic regression model containing main effects for change in log-transformed cytokines (except CYTO17) from LF to W1, we see significant coefficients for CYTO2 and CYTO9 at the $\alpha = 0.1$ level. We can interpret the coefficients as follows (using CYTO9 as example): for every 1 unit increase in the change of CYTO9, the odds of being IRI+ increase by 0.25 times (decrease by 75%).
...
A logistic model with random intercept per patient was also ran. This model aims to capture variation within each subject for each timepoint. We see that none of the predictors are significant. It is important to note that the method used to run this model may be biased, and the results should be taken with some skepticism. 
```{r}
library(lme4)
library(MASS)
...
m1 <- polr(Inflammation ~ CYTO1 + CYTO2 + CYTO3 + CYTO4 + CYTO5 + CYTO6 + CYTO7 
           + CYTO8 + CYTO9 + CYTO10 + CYTO11 + CYTO12 + CYTO13 + CYTO14 + CYTO15 
           + CYTO16 + CYTO18 + CYTO19 + CYTO20 + CYTO21 + CYTO22 + CYTO23, 
           data = dataPO, Hess=TRUE)
summary(m1)
...
m3 <- glm(Inflammation ~ CYTO1 + CYTO2 + CYTO3 + CYTO4 + CYTO5 + CYTO6 + CYTO7 
          + CYTO8 + CYTO9 + CYTO10 + CYTO11 + CYTO12 + CYTO13 + CYTO14 + CYTO15 
          + CYTO16 + CYTO18 + CYTO19 + CYTO20 + CYTO21 + CYTO22 + CYTO23, 
          data = dataPO1, family = "binomial")
summary(m3)
...
# ordinal logistic regression model
m2 <- polr(Necrosis ~ CYTO1 + CYTO2 + CYTO3 + CYTO4 + CYTO5  + CYTO6 + CYTO7 
           + CYTO8 + CYTO9 + CYTO10 + CYTO11 + CYTO12 + CYTO13 + CYTO14 + CYTO15 
           + CYTO16 + CYTO18 + CYTO19 + CYTO20 + CYTO21 + CYTO22 + CYTO23, 
           data = dataPO, Hess=TRUE)
summary(m2)
...
#binary response model
m4 <- glm(Necrosis ~ CYTO1 + CYTO2 + CYTO3 + CYTO4 + CYTO5 + CYTO6 + CYTO7 
          + CYTO8 + CYTO9 + CYTO10 + CYTO11 + CYTO12 + CYTO13 + CYTO14 + CYTO15 
          + CYTO16 + CYTO17 + CYTO18 + CYTO19 + CYTO20 + CYTO21 + CYTO22 + CYTO23, 
          data = dataPO1, family = "binomial")
summary(m4)

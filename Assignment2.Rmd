...
glimpse(BPD_tble)
>>>>>>> 6f8e57c8ff9ec2d07cd683f16d2763d9ce1f73b8
...
=======
  mutate(gender = ifelse(BPD_tble$sex == 1, "male", "female")) %>%  
  select(gender, bxy, agar, gage, bwt, rage, vage, cens) %>% 
  tbl_summary( by = cens,
    statistic = c("agar","gage","bwt", "rage", "vage") ~ "{median} ({p25}, {p75})",
    digits = all_continuous() ~ 1,
    label = list(bxy ~ "Birth Year (Range:62 to 73)", agar ~ "1-minute APGAR SCore", rage ~ "Age Onset of Respiratory Symptoms (Hoursx10)",

...
*2a.* Briefly outline statistical procedures that may help answer the four main statistical questions listed on Page 105 of Wolfe's casebook study.
...
# bpd
BPD_tble %>% 
  mutate(lbpd = as.factor(lbpd)) %>% 
  ggplot(mapping = aes(lbpd)) +
  geom_bar(aes(fill = lbpd), colour = "black") +
  scale_fill_brewer(palette = "Pastel1") +
  labs(x = "Stage of BPD Diagnosed from Lung Tissue", y = "Count",
...
We see that in the bpd data set, 7 variables (`brmu`, `puin`, `hyme`. `alin`, `emph`, `lbpd`, `hema`) have more than 60 missing values.

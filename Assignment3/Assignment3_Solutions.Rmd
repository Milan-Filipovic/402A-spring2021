...
* EBCT can measure changes in the extent of CAC in adults treated with lipid-lowering agents.
    +EBCT is fast, snesitive, and uses an electron gun instead of x-rays to scan the chest. 
    +Considered low risk and uses very low amounts of radiation. 
...
Next, let us visualize key predictors in the dataset.
...
When consulting the researhcers of this study, validity of outliers should be adressed. In adressing non-normality and zero inflation, researchers may want to consider selecting a different pool of participants who may have CAC growth. Atlernatively, scores can be converted into a categorically based risk system  such as the existing defined classification system for CAC scores.